<template>
  <div ref="containerRef" class="chart">
    ...
    <!-- legend -->
    <ul class="legend">
      <li><span class="sw sw--income" />{{ labels.income }}</li>
      <li><span class="sw sw--expense" />{{ labels.expense }}</li>
      <li><span class="sw sw--net" />{{ labels.net }}</li>
    </ul>

    <div v-if="hover >= 0 && data[hover]" class="tooltip" :style="tooltipStyle">
      <p class="tooltip-title">{{ data[hover].label }}</p>
      <p class="tip-row">
        <span class="sw sw--income" />{{ labels.income }}:
        {{ formatter(data[hover].income, currency) }}
      </p>
      <p class="tip-row">
        <span class="sw sw--expense" />{{ labels.expense }}:
        {{ formatter(data[hover].expense, currency) }}
      </p>
      <p class="tip-row">
        <span class="sw sw--net" />{{ labels.net }}: {{ formatter(data[hover].net, currency) }}
      </p>
    </div>
  </div>
</template>
...
const props = defineProps({
  ...
  labels: {
    type: Object,
    default: () => ({ income: 'Income', expense: 'Expense', net: 'Net' })
  }
});

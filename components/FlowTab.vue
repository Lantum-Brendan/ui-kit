  ...
  <section class="flow-tab">
    ...
    <div class="summary-strip">
      <article class="sum sum--in">
        ...
        <div>
          <p class="sum-label">{{ labels.totalIn }}</p>
      ...
      <article class="sum sum--out">
        ...
        <div>
          <p class="sum-label">{{ labels.totalOut }}</p>
      ...
      <article class="sum sum--save" :class="{ 'is-deficit': flow.savings <= 0 }">
        ...
        <div>
          <p class="sum-label">{{ flow.savings >= 0 ? labels.saved : labels.shortfall }}</p>
    ...
    <div class="canvas">
      <SankeyFlow :flow="flow" :currency="currency" :formatter="formatter" :labels="sankeyLabels" />
    </div>
...
const props = defineProps({
  ...
  labels: {
    ...
    default: () => ({
      flow: 'Flow',
      howMoneyMoved: 'How money moved',
      incomeOnLeft: 'Income on the left, where it went on the right. Hover a stream to focus.',
      totalIn: 'Total in',
      totalOut: 'Total out',
      saved: 'Saved',
      shortfall: 'Shortfall',
  ...
  sankeyLabels: {
    type: Object,
    default: () => ({
      empty: 'Not enough data to draw a flow yet',
      savings: 'Savings'
    })
  }
});

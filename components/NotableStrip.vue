<template>
  <section class="strip">
    ...
    <article class="note-card note-card--swing">
      ...
      <p v-else class="note-empty">{{ labels.noComparableChanges }}</p>
    </article>

    <article class="note-card note-card--first">
      <header>
        <span class="ico"><UserPlus :size="14" /></span>
        <h3>{{ labels.firstTimePayees }}</h3>
      </header>
      ...
      <p v-else class="note-empty">{{ labels.allFamiliarNames }}</p>
    </article>

    <article class="note-card note-card--streak">
      <header>
        <span class="ico"><ShieldCheck :size="14" /></span>
        <h3>{{ labels.noSpendStreak }}</h3>
      </header>
      <p class="note-value">
        {{ notable.longestNoSpendStreak.days }}
        <span class="value-suffix">{{ labels.days }}</span>
      </p>
      ...
      <p v-else class="note-empty">{{ labels.everyDayHadSpending }}</p>
    </article>
  </section>
</template>
...
const props = defineProps({
  ...
  labels: {
    ...
    default: () => ({
      ...
      noComparableChanges: 'No comparable changes',
      firstTimePayees: 'First-time payees',
      allFamiliarNames: 'All familiar names',

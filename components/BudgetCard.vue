  ...
  >
    ...
    <div class="scope-line">
      <span v-if="!hasAnyTargets" class="scope-pill scope-pill--all">
        ...
        {{ labels.allTransactionsInPeriod }}
      </span>
      <template v-else>
    ...
    <div class="progress">
      ...
      <div class="progress-meta">
        <span>
          ...
          {{ labels.of }} {{ formatMoney(effectiveLimit) }}
        </span>
    ...
    <footer class="budget-card__footer">
      <div class="footer-left">
        <span class="remaining-label">{{ labels.remaining }}</span>
        <strong class="remaining-value" :class="{ negative: remaining < 0 }">
          {{ formatMoney(remaining) }}
        </strong>
      </div>
      <div v-if="refunds > 0" class="footer-right" :title="labels.refundsAppliedThisPeriod">
...
const targets = computed<any[]>(() => props.budget.targets ?? []);
...
const summaryFor = (type: 'category' | 'group' | 'wallet', items: any[]) => {
...
const formatPeriod = (b: any) => {
  ...
  return `${label} · ${progress.value?.period_start ?? b.start_date}`;
};
</script>

  ...
  <section class="hero">
    <article class="hero-net" :class="netToneClass">
      <header class="net-head">
        <span class="net-label">{{ labels.netForPeriod }}</span>
        <span class="net-pill" :class="netToneClass">
          {{ totals.net >= 0 ? labels.surplus : labels.deficit }}
        </span>
      </header>
      ...
      <p v-if="compareEnabled" class="whisper-line">
        {{ labels.vs }} {{ format(totals.prevNet) }}
        <span class="whisper-meta">{{ labels.previousPeriod }}</span>
      </p>
      <div class="net-spark">
    ...
    <div class="sats">
      <article class="sat">
        <header class="sat-head">
          ...
          <span class="sat-label">{{ labels.income }}</span>
        </header>
        ...
        <p v-if="compareEnabled" class="whisper-line">
          {{ labels.vs }} {{ format(totals.prevIncome) }}
        </p>
        <div class="sat-spark">
      ...
      <article class="sat">
        <header class="sat-head">
          ...
          <span class="sat-label">{{ labels.expense }}</span>
        </header>
        <div class="sat-value-row">
        ...
        <p v-if="compareEnabled" class="whisper-line">
          {{ labels.vs }} {{ format(totals.prevExpense) }}
        </p>
        <div class="sat-spark">
      ...
      <article class="sat">
        <header class="sat-head">
          ...
          <span class="sat-label">{{ labels.savingsRate }}</span>
        </header>
        <div class="sat-value-row">
        ...
        <p v-if="compareEnabled" class="whisper-line">
          {{ labels.vs }} {{ (totals.prevSavingsRate * 100).toFixed(0) }}%
        </p>
        <div class="sat-spark">
...
const props = defineProps({
  ...
  labels: {
    type: Object,
    default: () => ({
      netForPeriod: 'Net for the period',
      surplus: 'Surplus',
      deficit: 'Deficit',
      vs: 'vs',

  ...
  <div :key="wallet.id" class="detail-body">
    <header class="hero surface" :class="netTone">
      <svg class="hero-decor" viewBox="0 0 800 80" preserveAspectRatio="xMidYMid slice" aria-hidden="true">
        <defs>
          <pattern :id="`whero-dots-${wallet.id}`" width="18" height="18" patternUnits="userSpaceOnUse">
      ...
      <button v-if="showBack" class="back-btn" :aria-label="labels.backToList" @click="$emit('back')">
        <ChevronLeft :size="18" />
      </button>
      ...
      <div class="hero-meta">
        <div class="hero-name-row">
          ...
          <span v-if="isDefault" class="hero-default-badge"><Star :size="11" /> {{ labels.default }}</span>
        </div>
        <span class="hero-sub"><span class="hero-currency">{{ wallet.currency }}</span><template v-if="wallet.type"><span class="hero-dot">·</span><span class="hero-type">{{ displayType }}</span></template></span>
      </div>
      <div class="hero-actions">
        <button class="hero-action" :title="labels.edit" :aria-label="labels.edit" @click="$emit('edit', wallet)"><Edit3 :size="14" /></button>
        <button class="hero-action hero-action--danger" :title="labels.delete" :aria-label="labels.delete" @click="$emit('delete', wallet)"><Trash2 :size="14" /></button>
      </div>
    </header>
    <section class="totals">
      <div class="total total--balance"><span class="total-label">{{ labels.balance }}</span><p class="total-value">{{ formatter(wallet.balance || 0, wallet.currency) }}</p></div>
      <div class="total-divider" />
      <div class="total total--income"><span class="total-label">{{ labels.income }}</span><p class="total-value tone-income">{{ formatter(wallet.stats?.total_income || 0, wallet.currency) }}</p></div>
      <div class="total-divider" />
      <div class="total total--expense"><span class="total-label">{{ labels.expense }}</span><p class="total-value tone-expense">{{ formatter(wallet.stats?.total_expense || 0, wallet.currency) }}</p></div>
      <div class="total-divider" />
      <div class="total total--net"><span class="total-label">{{ labels.net }}</span><p class="total-value" :class="netValue >= 0 ? 'tone-income' : 'tone-expense'">{{ formatter(netValue, wallet.currency) }}</p></div>
    </section>
    <section class="chart-card">
      <header class="chart-head"><div><span class="chart-eyebrow">{{ labels.last6Months }}</span><h3 class="chart-title">{{ labels.activity }}</h3></div><span class="chart-legend"><span class="legend-dot legend-dot--income" />{{ labels.income }}<span class="legend-dot legend-dot--expense" />{{ labels.expense }}</span></header>
      <svg v-if="hasActivity" class="chart-svg" :viewBox="`0 0 ${chartW} ${chartH}`" preserveAspectRatio="none">
        ...
        <polyline :points="linePoints('income')" fill="none" stroke="var(--color-income)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" vector-effect="non-scaling-stroke" />
        <polyline :points="linePoints('expense')" fill="none" stroke="var(--color-expense)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" vector-effect="non-scaling-stroke" />
        <g v-for="(m, i) in monthly" :key="`pt-${i}`"><circle v-if="m.income > 0" :cx="xAt(i)" :cy="yAt(m.income)" r="3" fill="var(--color-income)" /><circle v-if="m.expense > 0" :cx="xAt(i)" :cy="yAt(m.expense)" r="3" fill="var(--color-expense)" /></g>
    ...
    <section class="recent">
      <header class="recent-head"><div><span class="recent-eyebrow">{{ labels.recentTransactions }}</span><h3 class="recent-title">{{ recentTransactions.length }} {{ labels.shown }}</h3></div><NuxtLink :to="`/transactions?walletId=${wallet.id}`" class="recent-link">{{ labels.viewAll }}<ChevronRight :size="14" /></NuxtLink></header>
      <ul v-if="recentTransactions.length" class="recent-list">
        <li v-for="tx in recentTransactions" :key="tx.id" class="recent-row">
          <div class="recent-row-left"><span class="recent-type" :class="tx.type === 'INCOME' || tx.type === 'income' ? 'tone-income' : 'tone-expense'"><component :is="tx.type === 'INCOME' || tx.type === 'income' ? ArrowDownLeft : ArrowUpRight" :size="14" /></span><div class="recent-text"><span class="recent-name">{{ tx.party || tx.description || tx.category || labels.transaction }}</span><span class="recent-date">{{ formatDate(tx.date || tx.datetime) }}</span></div></div>
          <span class="recent-amount" :class="tx.type === 'INCOME' || tx.type === 'income' ? 'tone-income' : 'tone-expense'">{{ formatter(parseAmount(tx.amount).value, wallet.currency) }}</span>
        </li>
      </ul>
...
import { Edit3, Trash2, Star, ChevronLeft, ChevronRight, ArrowDownLeft, ArrowUpRight, Activity } from 'lucide-vue-next';
...
const displayType = computed(() => {
  ...
  const map = props.labels.walletType;
  return map[type] || props.wallet.type.charAt(0).toUpperCase() + props.wallet.type.slice(1).toLowerCase();
});
const netValue = computed(() => Number(props.wallet.stats?.total_income || 0) - Number(props.wallet.stats?.total_expense || 0));
...
const resolvedIcon = computed(() => {
  ...
  return (v && lucideMap[v]) || lucideMap.Wallet;
});
const walletTransactions = computed(() => props.transactions.filter((tx) => tx.walletId === props.wallet.id).sort((a, b) => new Date(b.date || b.datetime).getTime() - new Date(a.date || a.datetime).getTime()));
...
const monthly = computed(() => {
  ...
  for (let i = 5; i >= 0; i -= 1) {
    ...
    out.push({ key: `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`, label: new Intl.DateTimeFormat(props.locale, { month: 'short' }).format(d), income: 0, expense: 0 });
  }
  walletTransactions.value.forEach((tx) => {
    ...
    if (tx.type === 'INCOME' || tx.type === 'income') bucket.income += v; else bucket.expense += v;
  });
  return out;
});
const maxMonthValue = computed(() => { let max = 0; monthly.value.forEach((m) => { max = Math.max(max, m.income, m.expense); }); return max || 1; });
...
.hero-name-row{display:inline-flex;align-items:center;gap:8px;min-width:0}
...
.hero-default-badge{display:inline-flex;align-items:center;gap:3px;padding:1px 6px 1px 5px;background:var(--glass-bg);border:1px solid $border-light;color:var(--surface-deep, var(--color-primary-dark));border-radius:6px;font-size:10px;font-weight:$font-bold;text-transform:uppercase;letter-spacing:0.04em;flex-shrink:0}
...
.hero-currency{text-transform:uppercase;letter-spacing:0.05em;font-weight:$font-bold;color:var(--surface-deep, var(--color-primary-dark));opacity:0.85;font-variant-numeric:tabular-nums}
...
.totals{display:grid;grid-template-columns:1fr auto 1fr auto 1fr auto 1fr;align-items:center;padding:10px 14px;border-bottom:1px solid $border-light}

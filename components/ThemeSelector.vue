<template>
  <div ref="rootRef" class="theme-selector">
    <button class="icon-button" :aria-label="labels.theme" :title="labels.theme" @click="toggle">
      ...
      <div v-if="isOpen" class="theme-dropdown">
        ...
        <div class="theme-list">
          ...
          >
            ...
            <span class="theme-name">{{ labels[option.label] }}</span>
...
const emit = defineEmits<{
  toggle: [];
  selectTheme: [value: 'light' | 'dark' | 'system'];
}>();
...
const triggerIcon = computed(() => {
  if (props.theme === 'system') return Monitor;
  return props.isDark ? Moon : Sun;
});

const toggle = () => emit('toggle');
const selectTheme = (value: 'light' | 'dark' | 'system') => emit('selectTheme', value);
</script>

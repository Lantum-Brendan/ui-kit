    ...
    <div v-if="open" class="modal-root" @click.self="$emit('close')">
      <div class="modal-panel" role="dialog" aria-modal="true">
        <header class="modal-head">
          <div class="head-id">
            ...
            <div>
              ...
              <p class="head-sub">
                {{ fill(labels.periodOverview, { n: bucket.count }) }}
              </p>
            </div>
          </div>
          <button class="close-btn" :aria-label="labels.close" @click="$emit('close')">
            <X :size="18" />
          </button>
        </header>
        ...
        <section class="kpi-row">
          <div class="kpi">
            <span class="kpi-label">{{ labels.total }}</span>
            <span class="kpi-value">{{ formatter(bucket.amount, currency) }}</span>
          </div>
          <div class="kpi">
            <span class="kpi-label">{{ labels.avgPerMonth }}</span>
            <span class="kpi-value">{{ formatter(avgPerMonth, currency) }}</span>
          </div>
          <div class="kpi">
            <span class="kpi-label">{{ labels.peakMonth }}</span>
            <span class="kpi-value">{{ peakMonthLabel || '—' }}</span>
          </div>
          <div class="kpi">
            <span class="kpi-label">{{ labels.vsPriorPeriod }}</span>
            <span
              class="kpi-value"
        ...
        <section class="trend-block">
          <h3 class="block-title">{{ labels.trendLast6Months }}</h3>
        ...
        <section class="block">
          <h3 class="block-title">
            {{ fill(labels.topKind, { kind: bucket.kind === 'income' ? labels.sources : labels.payees }) }}
          </h3>
          ...
          <p v-else class="empty">{{ labels.noTransactionsMatch }}</p>
        </section>

        <section class="block">
          <h3 class="block-title">{{ labels.transactions }}</h3>
          ...
          >
            {{ fill(labels.showMore, { n: Math.min(10, filteredTxs.length - visibleCount) }) }}
          </button>
        </section>
      </div>
    </div>
...
const props = defineProps({
  ...
  bucket: { type: Object, default: () => ({ name: '', count: 0, amount: 0, delta: 0, trend: [], color: '#3b82f6' }) },
  range: { type: Object, default: null },
  transactions: { type: Array, default: () => [] },
  trailingMonths: { type: Array, default: () => [] },
...
const monthLabels = computed(() =>
  props.trailingMonths.map((m) =>
    new Intl.DateTimeFormat('en-US', { month: 'short' }).format(new Date(m.month + '-01'))
  )
);

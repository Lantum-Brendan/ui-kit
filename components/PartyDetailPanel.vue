      ...
      <div :key="party.id" class="detail-body">
        <header class="hero surface" :class="netTone">
          ...
          <div class="hero-actions">
            <button class="hero-action" :aria-label="labels.edit" @click="$emit('edit', party)">
              <Edit3 :size="14" />
            </button>
            <button
        ...
        <section class="chart-card">
          <header class="chart-head">
            <div>
              <span class="chart-eyebrow">{{ labels.lastSixMonths }}</span>
              <h3 class="chart-title">{{ labels.activity }}</h3>
            </div>
            <span class="chart-legend">
              <span class="legend-dot legend-dot--income" />{{ labels.received }}
              <span class="legend-dot legend-dot--expense" />{{ labels.spent }}
            </span>
          </header>
          <svg
            v-if="hasActivity"
        ...
        <section class="recent">
          <header class="recent-head">
            <div>
              <span class="recent-eyebrow">{{ labels.recentTransactions }}</span>
              <h3 class="recent-title">{{ recentTransactions.length }} {{ labels.shown }}</h3>
            </div>
            <button type="button" class="recent-link" @click="$emit('view-all', party.id)">
              {{ labels.viewAll }}
              ...
              <div class="recent-row-left">
                ...
                <div class="recent-text">
                  <span class="recent-name">{{
                    tx.category || tx.description || labels.transaction
                  }}</span>
...
const typeLabels = computed(() => ({
  individual: props.labels.individual,
  business: props.labels.business,
  organization: props.labels.organization,
  vendor: props.labels.vendor,
  client: props.labels.client
}));

const displayType = computed(() => {
  const type = props.party?.type?.toLowerCase();
  return typeLabels.value[type] || (props.party?.type ? props.party.type : props.labels.party);
});
...
const monthly = computed(() => {
  ...
  for (let i = 5; i >= 0; i -= 1) {
    ...
    out.push({
      ...
      date: d,
      label: new Intl.DateTimeFormat(props.locale, { month: 'short' }).format(d),
      receivedAmount: 0,
      spentAmount: 0
    });
  }
...
const parseAmount = (val) => {
  if (val == null) return { value: 0, currency: props.currency };
  if (typeof val === 'object' && 'value' in val) return val;
  const num = typeof val === 'string' ? Number(val.replace(/[^0-9.-]/g, '')) : Number(val);
  return { value: Number.isFinite(num) ? num : 0, currency: props.currency };
};

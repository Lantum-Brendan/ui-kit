<template>
  <div class="sessions-list">
    <h3 class="sessions-list__title">{{ labels.recentImports }}</h3>
    <ul class="sessions-list__items">
      ...
      >
        ...
        <div class="session-row__main">
          ...
          <span class="session-row__meta">
            ...
            <template v-if="suggestionCount(session) !== null">
              &middot; {{ suggestionCount(session) }} {{ labels.suggestions }}
            </template>
            <template v-if="duplicateCount(session) > 0">
              &middot; {{ duplicateCount(session) }} {{ labels.duplicates }}
            </template>
          </span>
        </div>
        ...
        <button
          ...
          :title="labels.deleteImport"
          :aria-label="labels.deleteImport"
...
interface ImportSession {
  id: number;
  file_name: string;
  status: 'pending' | 'analyzing' | 'extracting' | 'enriching' | 'checking' | 'ready' | 'confirmed' | 'failed' | 'expired';
  created_at: string;
  metadata?: {

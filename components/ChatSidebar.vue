<template>
  <aside class="sessions-pane">
      ...
      <div v-for="group in groupedSessions" :key="group.label" class="session-group">
        ...
        <ul>
          <li
            ...
            :title="session.title || _('newChat')"
            @click="$emit('open-session', session.id)"
          >
            <MessageSquare :size="14" class="session-icon" />
            <span class="session-title">{{ session.title || _('newChat') }}</span>
            <button
              class="remove-btn"
              :title="_('deleteChat')"
    ...
    <div v-else-if="!isLoading" class="empty-sessions">
      ...
      <p>{{ _('noConversations') }}</p>
      <span>{{ _('chatsWillAppear') }}</span>
    </div>
  </aside>
</template>
...
const props = withDefaults(
  defineProps<{
    ...
    isLoading: boolean;
    labels?: Partial<Record<'newChat' | 'deleteChat' | 'noConversations' | 'chatsWillAppear', string>>;
  }>(),
  {
    labels: () => ({
      newChat: 'New chat',
      ...
      chatsWillAppear: 'Your chats will appear here'
    })
  }
);

const _ = (k: 'newChat' | 'deleteChat' | 'noConversations' | 'chatsWillAppear') =>
  props.labels?.[k] ?? '';

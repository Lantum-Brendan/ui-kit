  ...
  <div class="period-control">
      ...
      <Transition name="pop">
        <div v-if="customOpen" class="custom-popover" @click.stop>
          ...
          <div class="popover-actions">
            <button class="ghost-btn" @click="customOpen = false">{{ labels.cancel }}</button>
    ...
    <div class="period-actions">
      <button
        ...
        :title="labels.compareToPrevious"
        @click="$emit('toggle-compare')"
      >
        <GitCompareArrows :size="14" />
        <span>{{ labels.compare }}</span>
      </button>
      <button class="primary-btn" :title="labels.openMonthlyRecap" @click="$emit('open-review')">
        <Sparkles :size="14" />
        <span>{{ labels.recap }}</span>
      </button>
    </div>
  </div>
</template>
...
const props = defineProps({
  ...
  labels: {
    ...
    default: () => ({
      ...
      compare: 'Compare',
      openMonthlyRecap: 'Open monthly recap',
      recap: 'Recap'
    })
  }
});

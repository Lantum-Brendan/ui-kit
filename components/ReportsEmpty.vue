  ...
  <section class="empty-card">
    <div class="empty-art" aria-hidden="true">
      <svg viewBox="0 0 240 160" width="200" height="140">
        ...
        <circle cx="130" cy="60" r="4" :fill="primary" />
        <circle cx="210" cy="40" r="5" :fill="primary" />
        <g class="dots">
          <circle cx="210" cy="40" r="10" :fill="primary" fill-opacity="0.18" />
        </g>
      </svg>
    </div>
    <h2 class="empty-title">{{ labels.title }}</h2>
    <p class="empty-text">{{ labels.text }}</p>
    <slot name="cta">
      <span class="cta">
        <Plus :size="16" />
...
defineProps({
  primary: { type: String, default: '#047844' },
  labels: {
    type: Object,
    default: () => ({
      title: 'Reports are waiting on data',
      text: 'Log a few transactions and your income, expenses, ratios, and patterns will appear here.',
...
.empty-art .dots {
  ...
  animation: pulse 2.2s $easing-emphasized infinite;
}

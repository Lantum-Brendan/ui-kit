  ...
  >
    <div class="canvas-head">
      ...
      <span class="canvas-text">
        <span class="canvas-title">{{ block.title || labels.document }}</span>
        <span class="canvas-meta">{{ labels.sectionCount.replace('{count}', String(sectionCount)) }}</span>
      </span>
    ...
    <div v-if="previewBlocks.length" class="canvas-preview">
      <template v-for="(b, i) in previewBlocks" :key="i">
        <div v-if="b.type === 'markdown'" class="canvas-markdown">{{ b.text }}</div>
        <ChatCalloutBlock
    ...
    <span class="canvas-cta">{{ labels.openInCanvas }}</span>
  </div>
</template>
...
import ChatTableBlock from './ChatTableBlock.vue';
...
const props = withDefaults(
  defineProps<{
    block: ChatBlock & { title?: string; blocks?: ChatBlock[] };
    labels?: {
      document?: string;
      sectionCount?: string;
      openInCanvas?: string;

<template>
  <section class="card">
    <header class="card-head">
      <div>
        <h2 class="card-title">{{ labels.spendingCalendar }}</h2>
        <p class="card-sub">
          {{ labels.eachTile }}
        </p>
      </div>
      <div class="legend">
        <span class="legend-label">{{ labels.less }}</span>
        <span v-for="step in 5" :key="step" class="legend-cell" :style="legendStyle(step - 1)" />
        <span class="legend-label">{{ labels.more }}</span>
      </div>
    </header>
    ...
    <footer v-if="hoveredSlot && !hoveredSlot.empty" class="hover-meta">
      ...
      <span class="hover-amt hover-amt--net">
        {{ labels.net }} {{ formatter(hoveredSlot.net, currency) }}
      </span>
    </footer>
  </section>
</template>
...
const props = defineProps({
  ...
  labels: {
    ...
    default: () => ({
      ...
      eachTile: 'Each tile is a day. Darker = more spent.',
      less: 'Less',
      more: 'More',
      net: 'Net'
    })

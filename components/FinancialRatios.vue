...
const props = defineProps({
  ...
  labels: {
    ...
    default: () => ({
      ...
      runway: 'Runway',
      savings: { healthy: 'Healthy', building: 'Building', watch: 'Watch', deficit: 'Deficit' },
      expense: { lean: 'Lean', steady: 'Steady', tight: 'Tight', over: 'Over' },
      runwayTone: { strong: 'Strong', ok: 'OK', short: 'Short', none: 'None' },
      months: '{n} months',
      dash: '—'
    })
  }
...
const ratios = computed(() => [
  {
    key: 'savings',
    label: props.labels.savingsRate,
    ...
    toneLabel:
      savingsRate.value >= 20
        ? props.labels.savings.healthy
        : savingsRate.value >= 10
          ? props.labels.savings.building
          : savingsRate.value >= 0
            ? props.labels.savings.watch
            : props.labels.savings.deficit
  },
  {
    key: 'expense',
    label: props.labels.expenseRatio,
    ...
    toneLabel:
      expenseRatio.value <= 70
        ? props.labels.expense.lean
        : expenseRatio.value <= 90
          ? props.labels.expense.steady
          : expenseRatio.value <= 100
            ? props.labels.expense.tight
            : props.labels.expense.over
  },
  {
    key: 'runway',
    label: props.labels.runway,
    formatted: runway.value > 0 ? fill(props.labels.months, { n: runway.value.toFixed(1) }) : props.labels.dash,
    tone:
      runway.value >= 6 ? 'good' : runway.value >= 3 ? 'ok' : runway.value >= 1 ? 'warn' : 'bad',
    toneLabel:
      runway.value >= 6
        ? props.labels.runwayTone.strong
        : runway.value >= 3
          ? props.labels.runwayTone.ok
          : runway.value >= 1
            ? props.labels.runwayTone.short
            : props.labels.runwayTone.none
  }
]);
</script>

import PartyDetailPanel from '../components/PartyDetailPanel.vue';

export default {
  title: 'Components/PartyDetailPanel',
  component: PartyDetailPanel,
  tags: ['autodocs'],
};

export const Default = { args: {
  party: { id: 1, name: 'Acme Corp', type: 'business', description: 'Primary client', receivedAmount: 1200, spentAmount: 400 },
  transactions: [{ id: 1, description: 'Grocery run', amount: 85, type: 'expense', date: '2026-08-10', currency: 'USD' }, { id: 2, description: 'Salary', amount: 3000, type: 'income', date: '2026-08-05', currency: 'USD' }],
  currency: 'USD',
  formatter: (n) => `$${Math.round(n)}`,
  showBack: false,
  locale: 'en'
} };
export const Empty = { args: {
  party: null,
  transactions: [],
  currency: '',
  showBack: false,
  locale: ''
} };
export const Alternative = { args: {
  party: { id: 1, name: 'Acme Corp', type: 'business', description: 'Primary client', receivedAmount: 1200, spentAmount: 400 },
  transactions: [{ id: 3, description: 'Alt expense', amount: 45, type: 'expense', date: '2026-08-12', currency: 'EUR' }],
  currency: 'EUR',
  formatter: (n) => `$${Math.round(n)}`,
  showBack: true,
  locale: 'fr'
} };
